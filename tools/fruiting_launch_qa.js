...
(async()=>{
  ...
  const baseline=JSON.parse(fs.readFileSync('docs/promotion-evidence/launch-qa.json','utf8'));
  for(let i=0;i<profiles.length;i++){const p=profiles[i],b=baseline.profiles[i];if(p.profile!==b.profile||p.maturity!==b.maturity||p.targetCount!==b.targetCount||p.rankedCount!==b.rankedCount||p.gisStatus!=='enhanced'||JSON.stringify(p.tiles)!==JSON.stringify(b.tiles))throw Error('Profile invariant failed: '+p.profile)}
