...
Preserve separate app, scoring and GIS versions. Bump the app footer for committed HTML changes using YYYY.MM.DD.N; current app version is 2026.09.20.6. There is no Junk Drawer registry in this standalone repository. Never change biology, profile mapping, weights, access or collecting semantics during infrastructure work.
...
Production uploads, manifest normalization and domain/CORS mutations need an explicit task scope. Read-only verification is preferred. Run relevant Python and Chrome Playwright tests; preserve current production and record accurate evidence. Cloudflare Pages serves the app at `fruitingforecast.com`; immutable GIS remains at `data.hanksjunkdrawer.com`. See docs/deployment.md before deploying.
